...
from cpbot import cpbot, Message, Config, get_collection, filters, pool

SAVED_SETTINGS = get_collection("CONFIGS")
GBAN_USER_BASE = get_collection("GBAN_USER")
WHITELIST = get_collection("WHITELIST_USER")
CHANNEL = cpbot.getCLogger(__name__)
LOG = cpbot.getLogger(__name__)
...
async def ungban_user(message: Message):
    ...
    if 'chat_ids' in found:
        for chat_id in found['chat_ids']:
            try:
                await cpbot.unban_chat_member(chat_id, user_id)
                await CHANNEL.log(
                    r"\\**#Antispam_Log**//"
                    f"\n**User:** [{firstname}](tg://user?id={user_id})\n"
                    f"**User ID:** `{user_id}`\n\n"
...
@cpbot.on_filters(filters.group & filters.new_chat_members, group=1, check_restrict_perm=True)
