...
from main_startup import bot, cpbot, Config
...
async def job_close():
    ...
    for warner in lol:
        try:
            await cpbot.send_message(
              int(warner.get("chat_id")), "`12:00 Am, Group Is Closing Till 6 Am. Night Mode Started !` \n**Powered By @cpbotOT**"
            )
            await cpbot.set_chat_permissions(warner.get("chat_id"), ChatPermissions())
            async for member in cpbot.iter_chat_members(warner.get("chat_id")):
             if member.user.is_deleted:
                try:
                    await cpbot.kick_chat_member(warner.get("chat_id"), member.user.id)
                except:
                    pass
            ...
            try:
                await cpbot.send_message(Config.LOG_GRP, f"[NIGHT MODE]\n\nFailed To Close The Group {ido}.\nError : {e}")
            except:
                pass
...
async def job_open():
    ...
    for warner in lol:
        try:
            ...
            await cpbot.set_chat_permissions(
                        warner.get("chat_id"),
                        ChatPermissions(
