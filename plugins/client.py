...
from cpbot import logging, Config, logbot
from cpbot.utils import time_formatter
from cpbot.utils.exceptions import cpbotBotNotFound
from cpbot.plugins import get_all_plugins
...
class _Abstractcpbot(Methods, RawClient):
    @property
    def id(self) -> int:
        if self.is_bot:
    ...
    async def load_plugin(self, name: str, reload_plugin: bool = False) -> None:
        """ Load plugin to cpbot """
        _LOG.debug(_LOG_STR, f"Importing {name}")
        _IMPORTED.append(
            importlib.import_module(f"cpbot.plugins.{name}"))
        if reload_plugin:
    ...
    def __eq__(self, o: object) -> bool:
        return isinstance(o, _Abstractcpbot) and self.id == o.id

    def __hash__(self) -> int:
        return super().__hash__()


class cpbotBot(_Abstractcpbot):
    """ cpbotBot, the bot """
    def __init__(self, **kwargs) -> None:
        _LOG.info(_LOG_STR, "Setting cpbotBot Configs")
...
class cpbot(_Abstractcpbot):
    """ cpbot, the userbot """

    has_bot = bool(Config.BOT_TOKEN)

    def __init__(self, **kwargs) -> None:
        _LOG.info(_LOG_STR, "Setting cpbot Configs")
        kwargs = {
        ...
        if Config.HU_STRING_SESSION and Config.BOT_TOKEN:
            ...
            kwargs['bot'] = cpbotBot(bot=self, **kwargs)
    ...
    @property
    def bot(self) -> Union['cpbotBot', 'cpbot']:
        """ returns cpbotbot """
        if self._bot is None:
            if Config.BOT_TOKEN:
                return self
            raise cpbotBotNotFound("Need BOT_TOKEN ENV!")
        return self._bot

    async def start(self) -> None:
        """ start client and bot """
        _LOG.info(_LOG_STR, "Starting cpbot")
        await super().start()
        if self._bot is not None:
            _LOG.info(_LOG_STR, "Starting cpbotBot")
    ...
    async def stop(self) -> None:  # pylint: disable=arguments-differ
        """ stop client and bot """
        if self._bot is not None:
            _LOG.info(_LOG_STR, "Stopping cpbotBot")
            await self._bot.stop()
        _LOG.info(_LOG_STR, "Stopping cpbot")
        await super().stop()
        _close_db()
    ...
    def begin(self, coro: Optional[Awaitable[Any]] = None) -> None:
        ...
        async def _shutdown(_sig: signal.Signals) -> None:
            ...
            _LOG.info(_LOG_STR, f"Received Stop Signal [{_sig.name}], Exiting cpbot ...")
            await _finalize()
