...
from cpbot import logging, Config
...
async def _init(r_c: Union['_client.cpbot', '_client.cpbotBot'],
...
async def _raise_func(r_c: Union['_client.cpbot', '_client.cpbotBot'],
...
async def _bot_is_present(r_c: Union['_client.cpbot', '_client.cpbotBot'],
...
async def _both_are_admins(r_c: Union['_client.cpbot', '_client.cpbotBot'],
                           r_m: RawMessage, is_bot: bool) -> bool:
...
async def _both_have_perm(flt: Union['types.raw.Command', 'types.raw.Filter'],
                          r_c: Union['_client.cpbot', '_client.cpbotBot'],
                          r_m: RawMessage, is_bot: bool) -> bool:
    if not await _bot_is_present(r_c, r_m, is_bot):
        return False
    try:
...
class RawDecorator(RawClient):
    ...
    def _build_decorator(self,
        ...
        def decorator(func: _PYROFUNC) -> _PYROFUNC:
            async def template(r_c: Union['_client.cpbot', '_client.cpbotBot'],
                               r_m: RawMessage) -> None:
                    ...
                    async with await _get_lock(str(flt)):
                        ...
                        if cond:
                            if Config.USE_USER_FOR_CLIENT_CHECKS:
                                if isinstance(r_c, _client.cpbotBot):
                                    return
