...
from cpbot import cpbot, Message, Config, logging, get_collection, pool
...
"""
    ...
    if thumb:
        args = {
            'caption': out_str,
            'parse_mode': "md",
            'reply_markup': markup if cpbot.has_bot else None
        }
    else:
        args = {
            'text': out_str,
    ...
    for chat_id in RSS_CHAT_ID:
        ...
        try:
            await send_rss_to_telegram(cpbot.bot, args, thumb)
        except (
            ChatWriteForbidden, ChannelPrivate, ChatIdInvalid,
            UserNotParticipant, cpbotBotNotFound
        ):
            ...
            await send_rss_to_telegram(cpbot, args, thumb)
