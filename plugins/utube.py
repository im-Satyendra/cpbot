...
from cpbot.utils import time_formatter, humanbytes
from .upload import upload

LOGGER = cpbot.getLogger(__name__)


@cpbot.on_cmd("ytinfo", about={'header': "Get info from ytdl",
...
@cpbot.on_cmd("ytdl", about={'header': "Download from youtube",
...
async def ytDown(message: Message):
    ...
    def __progress(data: dict):
        ...
        if data['status'] == "downloading" and (not edited or diff >= Config.EDIT_SLEEP_TIMEOUT):
            ...
            cpbot.loop.create_task(message.edit(out))
...
@cpbot.on_cmd("ytdes", about={'header': "Get the video description",
                               'description': 'Get information of the link without downloading',
